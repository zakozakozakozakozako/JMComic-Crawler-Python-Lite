...
class JmModuleConfig:
    ...
    def new_postman(cls, session=False, **kwargs):
        postman_type = kwargs.pop('type', 'requests')
        kwargs.setdefault('headers', JmModuleConfig.new_html_headers())
        kwargs.setdefault('proxies', JmModuleConfig.DEFAULT_PROXIES)

        # Termux/Android 上优先使用 requests 实现，避免依赖 curl_cffi 的原生轮子。
        if postman_type == 'requests':
            from .jm_postman import RequestsPostman
            return RequestsPostman(**kwargs)

        # 兼容旧配置：如用户显式指定了其它 postman 实现，则继续尝试 commonX。
        try:
            from common import Postmans
            kwargs.setdefault('impersonate', 'chrome')
            kwargs['type'] = postman_type
            if session:
